...
Sections:
  ...
  4. Try it yourself — enter readings for a new cycle and get both a
     ...
     consecutive cycles -- otherwise one new reading only shifts a 20-cycle
     rolling average by 1/21, which barely moves the forecast probability
     and makes the demo look unresponsive.
    ...
    with col1:
        ...
        persist_cycles = st.slider(
            ...
            help="A single one-off reading barely moves a 20-cycle rolling average -- that's correct "
                 "behavior (one weird part shouldn't trigger a drift alarm). Raise this to simulate the "
                 "tool holding this reading for several cycles in a row, the way real tool wear behaves, "
                 "and watch the forecast probability respond."
        )
