...
Sections:
  ...
  4. Try it yourself — enter readings for a new cycle and get both a
     detection verdict and a forecast alert
...
with tab2:
    ...
    st.caption("Logistic Regression, same-cycle features, 80/20 stratified split")
    c1, c2, c3 = st.columns(3)
...
with tab4:
    ...
    height_rolling_mean = (recent["crimp_height"].sum() + crimp_height) / (ROLL_WINDOW + 1)
    height_rolling_std = pd.concat([recent["crimp_height"], pd.Series([crimp_height])]).std()
    peak_force_rolling_mean = (recent["peak_force"].sum() + peak_force) / (ROLL_WINDOW + 1)
    pull_force_rolling_mean = (recent["pull_force"].sum() + pull_force) / (ROLL_WINDOW + 1)

    with col2:
        st.write("**Derived trend features (auto-computed from last 20 cycles + this input):**")
